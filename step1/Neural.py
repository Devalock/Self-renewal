...
np.savetxt(direct + 'CVrocNeural.txt', np.array(judge_final)/(5*re), fmt='%.3f',delimiter='\t')
with open(direct + 'CVrocNeural.txt',"a") as f_out:
	f_out.write('%.3f\t' %(sum_auc/(5*re)))

f = open(direct + 'CVProbNeural.txt','w')
