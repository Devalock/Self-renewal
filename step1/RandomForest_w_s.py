...
np.savetxt(direct + 'RF_CVroc_w_s.txt', np.array(judge_final) / (5 * re), fmt='%.3f', delimiter='\t')
with open(direct + 'RF_CVroc_w_s.txt', "a") as f_out:
    f_out.write('%.3f\t' % (sum_auc / (5 * re)))

f = open(direct + 'RF_CVProb.txt','w')
print (info)
