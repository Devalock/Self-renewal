...
for train, test in kf.split(feature):
	...
	model_train = 1 / (1 + exp(-model_train))
	model_test = feature[test]
	model_test = 1 / (1 + exp(-model_test))
	model_train_tag = tag[train]
	model_test_tag = tag[test]

	clf = svm.SVC(kernel='rbf',gamma='auto',C=1.0,probability=True)
...
np.savetxt(direct + 'SVM_CVroc_w_s.txt', np.array(judge_final)/(5*re), fmt='%.3f',delimiter='\t')
with open(direct + 'SVM_CVroc_w_s.txt',"a") as f_out:
	f_out.write('%.3f\t' %(sum_auc/(5*re)))

f = open(direct + 'SVM_CVProb.txt','w')
