...
np.savetxt(direct + 'LGBM_CVroc_w_s.txt', np.array(judge_final)/(5*re), fmt='%.3f',delimiter='\t')
with open(direct + 'LGBM_CVroc_w_s.txt',"a") as f_out:
	f_out.write('%.3f\t' %(sum_auc/(5*re)))

f = open(direct + 'LGBM_CVProb.txt','w')
